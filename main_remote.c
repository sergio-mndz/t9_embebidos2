...
static TaskHandle_t receiveTemperature_task_handle = NULL;

SemaphoreHandle_t getTemperature_mutex;
SemaphoreHandle_t temperatureObtained_mutex;
...
static void app_task(void *param)
{
    ...
    while (msg.DATA <= 10000U)
    {
        (void)PRINTF("Waiting for counter...\r\n");
        (void)rpmsg_queue_recv(my_rpmsg, my_queue, (uint32_t *)&remote_addr, (char *)&msg, sizeof(THE_MESSAGE),
                               ((void *)0), RL_BLOCK);
        if((msg.DATA % 5) == 0)
        {
        	xSemaphoreGive(getTemperature_mutex);
        	xSemaphoreTake(temperatureObtained_mutex, portMAX_DELAY);
...
void receiveTemperature_task(void *param)
{
	xSemaphoreTake(getTemperature_mutex, portMAX_DELAY);
	g_CurrentTemperature = LPADC_MeasureTemperature(DEMO_LPADC_BASE, DEMO_LPADC_USER_CMDID, 0U);
	xSemaphoreGive(temperatureObtained_mutex);
}
...
int main(void)
{
	getTemperature_mutex = xSemaphoreCreateMutex();
	temperatureObtained_mutex = xSemaphoreCreateMutex();
    ...
    if (xTaskCreate(receiveTemperature_task, "RECEIVETEMPERATURE_TASK", APP_TASK_STACK_SIZE, NULL, tskIDLE_PRIORITY + 1U, &receiveTemperature_task_handle) != pdPASS)
    {
        (void)PRINTF("\r\nFailed to create temperature task\r\n");
        for (;;)
        {
        }
    }
